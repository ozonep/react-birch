...
import { ITreeItemRaw, ITreeItem } from './ITreeItem';
...
export interface IBirchCoreHost<T> extends ITreeDataProvider<T> {
	 ...
	 *
	 * Hereafter, use `BirchRoot#inotify` to dispatch the events (dispatch a properly formatted `IBirchWatcherEvent` and `BirchRoot` will take care of the rest)
	 *
...
export interface IBirchWatcherChangeEvent {
	type: EnumBirchWatchEvent.Changed
	 ...
	 /**
	 * Unique id of the item that changed
	 */
	tid?: string
...
export interface IBirchWatcherRemoveEvent {
	...
	/**
	 * Unique id of the item that is no longer existent
	 */
	tid?: string
...
export interface IBirchWatcherMoveEvent {
	...
	/**
	 * Unique id of the item that moved
	 */
	tid?: string
