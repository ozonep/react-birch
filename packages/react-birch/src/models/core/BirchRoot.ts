...
export class BirchRoot extends BirchFolder {
	...
	constructor(host: IBirchCoreHost<any>, root: string) {
		...
		super(null, superv, null, null, "ROOT", null)
		this._pathfx = pathfx
		this.host = host
		this.rootPath = root
	...
	public inotify(event: IBirchWatcherEvent): void {
		switch (event.type) {
			...
			case EnumBirchWatchEvent.Changed:
				return this.queueChangeEvent(event.folder)
	...
	public getIndexAtItemEntry(itemEntry: BirchItemOrFolder) {
		return this.flattenedBranch.indexOf(itemEntry.id)
	}
	...
	public setExpanded(_ensureVisible?: boolean): Promise<void> {
	...
	public async flushEventQueue() {
		...
		const promise = pSeries(this.changeEventDispatchQueue.map((path) => async () => {
			...
			if (watcher && typeof watcher.callback === 'function') {
				await watcher.callback({ type: EnumBirchWatchEvent.Changed, folder: path })
			}
			return null
		}) as any)
		// reset the queue
