...
import { IBirchWatcherEvent, EnumBirchWatchEvent, BirchWatchTerminator } from '../../types/birch'
...
export class BirchFolder extends BirchItem implements ITreeItemRaw {
	...
	protected constructor(root: BirchRoot, tree: IBirchTreeSupervisor, parent: BirchFolder, folderName: string, tid: string, details: ITreeItem) {
		super(root, tree, parent, folderName, tid, details)
	...
	public insertItem(item: BirchItem | BirchFolder) {
		...
		this._children.sort(this.root.host.sortComparator || BirchFolder.defaultSortComparator)
	...
	protected async hardReloadChildren() {
		...
		const rawItemsPre = await this.root.host.getChildren({ path: this.path, tid: this._tid }) || []
		...
		for (let i = 0; i < rawItems.length; i++) {
			...
			const child = new (type === EnumTreeItemType.Folder ? BirchFolder : BirchItem)(this.root, this._superv, this, label, tid, rawTreeItem)
	...
	private handleWatchEvent = async (event: IBirchWatcherEvent) => {
		...
		if (event.type === EnumBirchWatchEvent.DidChangeTreeData) {
			...
			const { item: rawBirchItem } = event
			BirchItem.checkRawItem(rawBirchItem)
			const newItem = new (rawBirchItem.type === EnumTreeItemType.Folder ? BirchFolder : BirchItem)(this.root, this._superv, this, rawBirchItem.label, rawBirchItem.tid, rawBirchItem /* to do call treedataprovider gettreeitem */)
			...
			// TODO: Try to "rehydrate" tree instead of hard reset (if possible) (maybe ITreeItemRaw can have optional `id` prop? hash of (ctime + [something])?)
			for (let i = 0; i < this._children.length; i++) {
